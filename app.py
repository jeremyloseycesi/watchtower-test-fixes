...
def get_user(user_id):
    ...
    if result:
        return jsonify({"user": {"id": result[0], "name": result[1], "email": result[2]}})
...
def read_log():
    ...
    try:
        ...
        if not str(file_path).startswith(str(logs_dir) + os.sep):
...
def is_admin(auth_token):
    ...
    # FIXED: Compare auth_token with admin_token (not admin_token with itself)
