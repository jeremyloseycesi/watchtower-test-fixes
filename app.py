...
def get_database_connection():
    ...
    # For SQLite, this is simple, but for production databases:
...
def require_admin(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Simple authentication check (in production, use proper auth system)
        if not auth_token or not is_admin(auth_token):
...
def is_admin(auth_token):
    ...
    return auth_token == admin_token if admin_token else False
...
def delete_user(user_id):
    """
    FIXED: Added authentication check before allowing deletion.
    Also fixed SQL injection in delete query.
    """
    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
